...
basedir = '/run/user/1000/gvfs/smb-share:server=isiserver.curie.net,share=u934/equipe_bellaiche/el_alpar/201026_DfdLife/ECadGFP/Projected/TO_SEGMENT'

basedir_StardistWater_FijiMask = basedir + '/StarWat_HPNeck5/'
basedir_StardistWater_FijiMask_RTGPipeline = basedir + '/StarWat_HPNeck5_RTGPipeline/'

Model_Dir = '/run/media/sancere/DATA/Lucas_Model_to_use/Segmentation/_HPTunning/'    
...
for fname in filesRaw:
    ...
    if os.path.exists((basedir_StardistWater_FijiMask + 'StarWat_HPNeck5' + os.path.basename(os.path.splitext(fname)[0]))) == False :
       ...
       # Binary_Watershed = Integer_to_border(Watershed)
        ...
        save_8bit_tiff_imagej_compatible((basedir_StardistWater_FijiMask + 'StarWat_HPNeck5' + os.path.basename(os.path.splitext(fname)[0]) ) , StardistWater_FijiMask, axes)
...
for fname in filesRaw_2:
    ...
    png.from_array(z, mode="L").save(basedir_StardistWater_FijiMask_RTGPipeline + name.replace("StarWat_HPNeck5_",'seg_') + '.png') 
