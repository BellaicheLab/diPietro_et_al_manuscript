...
basedir = '/run/user/1000/gvfs/smb-share:server=isiserver.curie.net,share=u934/equipe_bellaiche/l_sancere/WildType_Management/wt_mov13_starwat_v2.3.1_3frames_corrected'

basedir_StardistWater_FijiMask = basedir + '/StarWat_PreviousHP/'
basedir_StardistWater_FijiMask_RTGPipeline =  basedir + '/StarWat_PreviousHP_RTGPipeline/'
...
for fname in filesRaw:
    ...
    if os.path.exists((basedir_StardistWater_FijiMask + 'StarWat_PreviousHP' + os.path.basename(os.path.splitext(fname)[0]))) == False :
       ...
       # Binary_Watershed = Integer_to_border(Watershed)
        ...
        save_8bit_tiff_imagej_compatible((basedir_StardistWater_FijiMask + 'StarWat_PreviousHP' + os.path.basename(os.path.splitext(fname)[0]) ) , StardistWater_FijiMask, axes)
...
for fname in filesRaw_2:
    ...
    png.from_array(z, mode="L").save(basedir_StardistWater_FijiMask_RTGPipeline + name.replace("StarWat_PreviousHP",'seg_') + '.png') 
